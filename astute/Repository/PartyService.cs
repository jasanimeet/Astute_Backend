...
namespace astute.Repository
{
    public partial class PartyService : IPartyService
    {
        ...
        public async Task<List<Dictionary<string, object>>> GetPartyName(int party_Id)
        {
            var result = new List<Dictionary<string, object>>();
            using (var connection = new SqlConnection(_configuration["ConnectionStrings:AstuteConnection"].ToString()))
            {
                using (var command = new SqlCommand("Party_Master_Name_Select", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.Add(party_Id > 0 ? new SqlParameter("@PartyId", party_Id) : new SqlParameter("@PartyId", DBNull.Value));
        ...
        public async Task<int> Add_Update_Customer_Party_File(Customer_Party_File party_File, int modified_By, string map_Flag)
        {
            ...
            var _API_URL = !string.IsNullOrEmpty(party_File.API_URL) ? new SqlParameter("@API_URL", party_File.API_URL) : new SqlParameter("@API_URL", DBNull.Value);

            var result = await Task.Run(() => _dbContext.Database
                        .ExecuteSqlRawAsync(@"EXEC Customer_Party_File_Insert_Update @File_Id, @User_Pricing_Id, @File_Name,@File_Type,@IP,@Country, @File_Status, @Modified_By, @Map_Flag, @API_URL",
                        file_Id, _user_Pricing_Id, file_Name, file_Type, iP, country, file_Status, _modified_By, _map_Flag, _API_URL));
        ...
        {
            var result = await Task.Run(() => _dbContext.DropdownModel
                            .FromSqlRaw(@"exec Get_All_Party_Supplier")
                            .ToListAsync());
            return result;
        }
        ...
        public async Task<List<Dictionary<string, object>>> Get_Supplier_Special_Price_Validity(int party_Id)
        {
            var result = new List<Dictionary<string, object>>();
            using (var connection = new SqlConnection(_configuration["ConnectionStrings:AstuteConnection"].ToString()))
            {
                using (var command = new SqlCommand("Supplier_Special_Price_Validity_Select", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.Add(party_Id > 0 ? new SqlParameter("@PartyId", party_Id) : new SqlParameter("@PartyId", DBNull.Value));
                    await connection.OpenAsync();
        ...
        public async Task<List<Dictionary<string, object>>> Get_Quotation_BillParty_Detail(int party_Id)
        {
            var result = new List<Dictionary<string, object>>();
            using (var connection = new SqlConnection(_configuration["ConnectionStrings:AstuteConnection"].ToString()))
            {
                using (var command = new SqlCommand("Quotation_BillParty_Detail", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
